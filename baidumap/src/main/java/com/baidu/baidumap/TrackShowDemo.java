...
import com.example.applibrary.ThreadPoolUtils;
import com.example.applibrary.ToastFactory;
import com.example.applibrary.rlPart.FileListAdapter;
...
public class TrackShowDemo extends AppCompatActivity implements OnGetGeoCoderResultListener {
    ...
    @BindView(R2.id.popup_rl)
    RelativeLayout popupRl;
    @BindView(R2.id.pop_iv)
    ImageView popupIv;
...
//        LogUtils.i(tag,"onGetReverseGeoCodeResult");
    ...
    public void popWindowShowFile(View v) {
        ...
        adapter.setOnClickListener(new com.example.applibrary.rlPart.base.BaseAdapter.OnClickListener() {
            @Override
            public void onShortClick(View v, int position) {
                if (v.getId() == R.id.file_tv){
                    pw.dismiss();
                ...
                }else if (v.getId() == R.id.right_menu){
        ...
        pw = new PopupWindow(popupView, ViewGroup.LayoutParams.MATCH_PARENT, DeviceInfoUtils.getScreenHeight(mContext) / 3, true);
//        pw.setFocusable(true);
//        pw.setOutsideTouchable(true);//没多大用
        pw.showAtLocation(popupRl, Gravity.NO_GRAVITY, location[0], location[1] - DeviceInfoUtils.getScreenHeight(mContext) / 3);
