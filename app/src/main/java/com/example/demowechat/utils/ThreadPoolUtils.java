...
public final class ThreadPoolUtils {
    ...
    private final String tag = "ThreadPoolUtils";
...
//    private ExecutorService          exec;
    ...
    public static ThreadPoolUtils getInstance(){
        if (instance == null){
            instance = new ThreadPoolUtils(SingleThread,1);
        }
        return instance;
    }
    ...
    public ThreadPoolUtils(@Type final int type, final int corePoolSize) {
        ...
        switch (type) {
            case FixedThread:
                // 构造一个固定线程数目的线程池
                exec = new ThreadPoolExecutor(corePoolSize, corePoolSize, 0L, TimeUnit.MILLISECONDS, new LinkedBlockingQueue<Runnable>());
...
//                exec = Executors.newCachedThreadPool();
    ...
    public void cancel(){
        ...
        if (mRunnable!= null) {
            LogUtils.i(tag,"cancle");
            exec.remove(mRunnable);
        }
    }
    public void cancel(final Runnable command){
